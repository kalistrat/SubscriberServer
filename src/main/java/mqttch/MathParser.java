...
/**
 * Created by kalistrat on 25.05.2017.
 */
public class MathParser {

    public static HashMap<String, Double> var;
    public static List<String> VarList = new ArrayList<>();

    public MathParser() {
        var = new HashMap<>();
        //setVariable("pi",Math.PI);
        //setVariable("exp",Math.E);
    }
    ...
    public Double getVariable(String varName) throws Exception {
        if(!var.containsKey(varName)) {
            throw new Exception("нет переменной|" + varName + "|");
        }
    ...
    public double Parse(String s) throws Exception {
        ...
        if (!result.rest.isEmpty())
            throw new Exception("Ошибка: нельзя распознать выражение "+
                    ": " + result.rest);
        return result.acc;
    }
    ...
    private Result num(String s) throws Exception{
        ...
        while (i < s.length() &&
            ...
            if (s.charAt(i) == '.' && ++dot_cnt > 1) {
                throw new Exception("Неверное число: избыточный разделитель "
                        + s.substring(0, i + 1));
            }
            i++;
        }
        if( i == 0 ){ // что-либо похожее на число мы не нашли
            throw new Exception("Неверное число в выражении " + s);
        }
    ...
    private Result processFunction(String func,
        ...
        switch(func){
            ...
            default:
                throw new Exception("функция " + func +
                        " не определена");
        }
    }
